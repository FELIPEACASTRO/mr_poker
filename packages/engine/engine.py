...
class GameEngine:
    ...
    def start_new_hand(
        ...
        *,
        ...
        if len(stacks) != 2:
            raise ValueError("Sprint 03 engine supports exactly 2 players")
        ...
        runtime = HandRuntime(state=state, deck=deck)

        self._deal_hole_cards(runtime)
        sb_seat = button_seat
        bb_seat = self._other_seat(button_seat)
        self._commit(runtime, sb_seat, self.small_blind)
        state.actions.append(
            ActionEvent(actor_seat=sb_seat, action_type=ActionType.POST_SMALL_BLIND, amount=self.small_blind)
        )
        self._commit(runtime, bb_seat, self.big_blind)
        state.actions.append(
            ActionEvent(actor_seat=bb_seat, action_type=ActionType.POST_BIG_BLIND, amount=self.big_blind)
        )
        state.current_bet = min(self.big_blind, state.players[bb_seat].invested_this_round)
        state.street_starting_seat = button_seat
        state.acting_seat = button_seat
        state.to_call = self._to_call_for_player(state.players[button_seat], state)
    ...
    def apply_action(self, runtime: HandRuntime, action_type: ActionType, amount: int = 0) -> HandState:
        ...
        if player.folded or player.is_all_in:
            raise ValueError("acting player is not eligible to act")
        to_call = self._to_call_for_player(player, state)

        if action_type == ActionType.FOLD:
            self._append_action(state, seat, action_type, 0)
            player.folded = True
            winner = self._other_active_seat(state, seat)
            self._award_entire_pot(state, winner)
            return state
        ...
        if action_type == ActionType.BET:
            if to_call != 0:
                raise ValueError("use raise when facing a bet")
            if amount < state.big_blind:
                raise ValueError("bet amount must be at least one big blind")
        ...
        if action_type == ActionType.RAISE:
            if to_call == 0:
                raise ValueError("use bet when opening the action")
        ...
        if action_type == ActionType.ALL_IN:
            ...
            if raise_to <= player.invested_this_round:
                raise ValueError("player has no chips left")
    ...
    def _deal_hole_cards(self, runtime: HandRuntime) -> None:
        state = runtime.state
        order = [state.button_seat, self._other_seat(state.button_seat), state.button_seat, self._other_seat(state.button_seat)]
        for seat in order:
    ...
    def _append_action(self, state: HandState, seat: int, action_type: ActionType, amount: int, note: str = "") -> None:
        state.actions.append(
            ActionEvent(actor_seat=seat, action_type=action_type, amount=amount, street=state.street, note=note)
        )
    ...
    def _raise_to(
        ...
        *,
        ...
        if not allow_short_raise and state.min_raise_to is not None and raise_to < state.min_raise_to:
            raise ValueError("raise amount is below minimum raise")
        needed = raise_to - player.invested_this_round
        if needed <= 0:
            raise ValueError("raise must increase player investment")
        if needed > player.stack:
            raise ValueError("raise exceeds player stack")
    ...
    def _resolve_after_action(self, runtime: HandRuntime) -> None:
        ...
        next_seat = self._other_active_seat(state, state.acting_seat)
        ...
        if len(state.acted_this_street) >= len(active):
    ...
    def _advance_street_or_showdown(self, runtime: HandRuntime) -> None:
        ...
        state.street_starting_seat = self._other_seat(state.button_seat)
        state.acting_seat = state.street_starting_seat
    ...
    def _other_active_seat(self, state: HandState, seat: int) -> int:
        for other in state.players:
            if other != seat and not state.players[other].folded:
                return other
        raise ValueError("no other active seat found")

    def _other_seat(self, seat: int) -> int:
        return 1 if seat == 0 else 0

    def _odd_chip_seat(self, state: HandState, winners: list[int]) -> int:
        preferred = self._other_seat(state.button_seat)
        return preferred if preferred in winners else winners[0]
    ...
    def _refund_uncalled_if_needed(self, state: HandState) -> int:
        ...
        state.actions.append(
            ActionEvent(actor_seat=seat, action_type=ActionType.CHECK, amount=refund, street=Street.SHOWDOWN, note="uncalled_return")
        )
        return refund
