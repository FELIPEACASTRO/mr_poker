...
def _spr_bucket(spr: float) -> str:
    if spr < 2: return 'spr_lt_2'
    if spr < 4: return 'spr_2_4'
    if spr < 8: return 'spr_4_8'
    return 'spr_ge_8'


def _equity_bucket(equity: float) -> str:
    if equity < 0.35: return 'eq_low'
    if equity < 0.5: return 'eq_medium'
    if equity < 0.65: return 'eq_good'
    return 'eq_strong'


def _price_bucket(edge: float) -> str:
    if edge < -0.08: return 'underpriced_bad'
    if edge < 0.0: return 'underpriced_close'
    if edge < 0.08: return 'priced_close'
    return 'priced_good'
